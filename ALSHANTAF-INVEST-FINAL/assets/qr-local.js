/*
 ...
 Browser bundle created from qrcode-terminal's vendored implementation.
*/
(function(global){
"use strict";
var modules={},cache={};
function define(id,fn){modules[id]=fn;}
function req(id){if(cache[id])return cache[id].exports;if(!modules[id])throw new Error("QR module not found: "+id);var module={exports:{}};cache[id]=module;modules[id](module,module.exports,req);return module.exports;}
...
var QRCodeModel=req('index');
var QRErrorCorrectLevel=req('QRErrorCorrectLevel');
function makeCanvas(text,size){
  size=size||260;
  var qr=new QRCodeModel(0,QRErrorCorrectLevel.M);
  qr.addData(String(text||''));
  qr.make();
  var count=qr.getModuleCount(),quiet=4,total=count+quiet*2,scale=Math.max(1,Math.floor(size/total));
  var canvas=document.createElement('canvas');
  canvas.width=canvas.height=total*scale;
  var ctx=canvas.getContext('2d');
  ctx.fillStyle='#fff';ctx.fillRect(0,0,canvas.width,canvas.height);
  ctx.fillStyle='#102f2a';
  for(var r=0;r<count;r++)for(var c=0;c<count;c++)if(qr.isDark(r,c))ctx.fillRect((c+quiet)*scale,(r+quiet)*scale,scale,scale);
  return canvas;
}
global.AlshantafQR={makeCanvas:makeCanvas,toDataURL:function(text,size){return makeCanvas(text,size).toDataURL('image/png');}};
})(window);
